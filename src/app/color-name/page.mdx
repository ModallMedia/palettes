...
<CodeGroup title="Making a request">
  ```bash
  curl "https://yourapi.domain.com/api/v1/color-name?hex=<HEX_CODE>"
  ```
  ```js
...
async function requestColorSchemes() {
  ...
  const url = encodeUriComponent(`https://yourapi.domain.com/api/v1/color-name?hex=${color}`);
...
def request_color_schemes():
    ...
    url = f"https://yourapi.domain.com/api/v1/color-name?hex={color}"
...
function requestColorSchemes() {
    $color = "#ff0000";  // Replace with your color value
    $url = "https://yourapi.domain.com/api/v1/color-name?hex=" . urlencode($color);
...
def request_color_schemes
  color = "#ff0000"  # Replace with your color value
  url = URI("https://yourapi.domain.com/api/v1/color-name?hex=#{URI.encode_www_form_component(color)}")
