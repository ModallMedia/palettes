...
<Row>
  ...
  <Col sticky>
    <CodeGroup title="Example Request" tag="GET" label="/api/v1/blend">
      ...
      ```bash
      curl "https://mogk.io/api/v1/blend-colors?colors=<HEX_ONE>&colors=<HEX_TWO>&colors=<HEX_THREE>"
      ```
      ...
      async function request() {
        ...
        const response = await fetch(
          `https://mogk.io/api/v1/blend-colors?${params.toString()}`
        );
      ...
      response = requests.get('https://mogk.io/api/v1/blend-colors', params={
      ...
      // PHP (using cURL)
      $ch = curl_init();
      curl_setopt($ch, CURLOPT_URL, "https://mogk.io/api/v1/blend-colors?colors=%23ff0000&colors=%2300ff00&colors=%230000ff");
      ...
      uri = URI('https://mogk.io/api/v1/blend-colors')
